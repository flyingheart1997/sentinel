...
import { openNewsDetailModal } from '@/components/NewsDetailModal';
...
export class NewsPanel extends Panel {
  ...
  private bindCardEvents(): void {
    const items = this.content.querySelectorAll<HTMLElement>('.item.clustered');
    items.forEach(item => {
      item.style.cursor = 'pointer';
      item.addEventListener('click', (e) => {
        // Only open modal if we didn't click on an inner interactive element
        if ((e.target as HTMLElement).closest('a, button, .related-asset')) return;

        const clusterId = item.dataset.clusterId;
        if (!clusterId) return;
        const cluster = this.latestClusters.find(c => c.id === clusterId);
        if (cluster) {
          e.preventDefault();
          e.stopPropagation();
          openNewsDetailModal(cluster);
        }
      });
    });
  }
