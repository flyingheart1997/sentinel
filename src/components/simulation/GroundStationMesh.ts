...
import { GroundStation } from '../../modules/satellites/types';
...
function createGSIconTexture(): THREE.Texture {
    ...
    // ── Parabolic dish bowl ──────────────────────────────────────
    // Draw as a bezier parabola: open at top, concave upward
    ...
    // Feed horn (small square at end of arm)
...
export class GroundStationLayer {
    ...
    constructor(
        ...
        // renderOrder=2 lets icons draw after the Earth but keeps depth test ON
        // so GS on the far side of Earth are correctly hidden by the globe
    ...
    tick(): void {
        ...
        // Always update real-time comm lines between ALL GS and nearest satellites
        ...
        if (selectedId) {
            ...
            if (gs) {
                ...
                // Deep zoom: 150 units above the GS surface
    ...
    private onClick(e: MouseEvent): void {
        ...
        // Dynamic threshold: scale with camera distance so hit-area matches visual icon size at all zooms
    ...
    private updateFocusedModel(pos: THREE.Vector3, gsId: string, surfaceNormal: THREE.Vector3): void {
        ...
        // Place the sprite at the same point as the GS icon
        // Sprites always face the camera so no rotation/quaternion needed
        this.focusedModel.position.copy(pos);
        void surfaceNormal; // unused for Sprite — kept for API compat
    }
    ...
    private createDishModel(): THREE.Group {
        ...
        // Selection indicator: two concentric pulsing rings around the GS point
        // Clean, professional selection ring — no glow blob
    ...
    private latLonToCartesian(lat: number, lon: number): THREE.Vector3 {
        ...
        // 250 units above surface so the icon floats clearly above the globe
